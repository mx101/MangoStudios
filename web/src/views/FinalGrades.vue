...
export default {
  ...
  methods: {
    ...
    updateData() {
        ...
        var dataCSV = "/final_grade/"

        dataCSV = dataCSV + "final_grade"
        ...
        if (this.student_behaviors.length == 0) {
            ...
            if (this.student_behaviors == "Autograders Used") {
                ...
                d3.csv(dataCSV).then( function(data) {
                    // Add X axis
                    const x = d3.scaleLinear()
                        .domain([0, d3.max(data, function(d) { return +d.potds })])
                        .range([ 0, width ]);
                        svg.append("g")
                            .attr("transform", `translate(0, ${height})`)
                            .call(d3.axisBottom(x))
                    ...
                    // Add Y axis
                    const y = d3.scaleLinear()
                        .domain([25 , d3.max(data, function(d) { return +d.score })])
                        .range([ height, 0]);
                        svg.append("g")
                        .call(d3.axisLeft(y))
                            .append("text")
                    ...
                    const annotations = [
                        {                      
                            note: 
                            {
                                ...
                                label: "Students who do more POTDs tend to do better on Exams.",
                                wrap: 125
                            }, 
                            x: 300,
                            y: 225,
