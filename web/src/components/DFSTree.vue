<template>
  <div>
    <div style="display: flex; justify-content: center; align-items: center;">
      <input name="input" class="form-control" type="number" step="1" min="-20" max="50" autocomplete="off" v-model="search_value" style="width: auto; font-size: 1em; text-align: center;"> 
      <button class="btn btn-primary" name="submit" type="submit" style="margin: 0 0.75em" value="Watch Search" v-on:click="animate_path">Watch DFS Search</button>
    </div>
...
class BinarySearchTree {
  ...
  insertNode(node, newNode) {
    var test = Math.floor(Math.random() * (1 - 0 + 1) + 0);
    console.log(test)
    if (test == 1) {
      if (node.left === null) {
        node.left = newNode
  ...
  dfsIteration(node, search_value) {
    if (this.found) {
      return
    }
    if (node == null) {
    ...
    this.dfsIteration(node.left, search_value)
    this.dfsIteration(node.right, search_value)
  }

  getDFS(search_value) {
    this.path = []
    this.found = false
    this.dfsIteration(this.root, search_value)
    return this.getPath()
  }
  ...
  methods: {
    ...
    animate_path() {
      ...
      var g = svg
        .select('g')
        .attr('transform', `translate(${margin},${margin})`);
      var path = this.tree_.getDFS(this.search_value);
      ...
      for(let k = 0; k < path.length - 1; k++) {
        ...
        .transition()
        .duration((path.length - 1)*500 + 1000 - k * 500)
        .transition()
        .duration(500)
        .style("opacity", 0)
        .style('font-size', '0px')
